...
object test {
  def main(args: Array[String]): Unit = {
    ...
    val testing = sdf
      ...
      .select(col("value.*"))
      .select(col("uid"), explode(col("visits.url")).alias("visit"))
      .withColumn("domain", regexp_replace(col("visit"), "(http://)?www.", ""))
      .groupBy(col("uid")).agg(collect_list(col("domain")).alias("domains"))
